...
# Example detectors - replace with your actual list
    ...
    all_detectors = [
        ...
        # ========================================================================
        # DETECTORS FROM THE MANUSCRIPT
        # ========================================================================
        
        # --- UNIVERSAL DETECTOR (Theorem 5.1) ---
        ('Universal', 
         '(n^2 - 5*n + 4)*M1^3 - 24*M1^2*M2 + (3*n^2 - 3)*M1^2 + (24*n + 24)*M1*M2 - (8*n^2 + 8*n + 8)*M2 - (2*n^3 - 2*n^2 + n - 1)*M1'),

        # --- HYBRID DETECTORS (Section 6) ---
        ('H_1,2', 
         '(n - 1)*M1 + 8*M2 - (n^3 - n^2 - n + 1)'),
         
        ('H_mix', 
         'M1^3 - (3*n + 3)*M1^2 + (3*n^2 + n + 4)*M1 - 8*M2'),
         
        ('H_1,5', 
         '64*M2^2 - 16*M1*M2 + (n^3 - 3*n^2 + n + 1)*M1^2 - (16*n^3 - 24*n^2 - 24*n + 16)*M2 - (3*n^4 - 6*n^3 - n^2 + 6*n - 2)*M1 + (n^6 - n^5 - n^4 + 2*n^3 - n^2 - n + 1)'),

        # --- MASTER THEOREM EXAMPLES (Section 4) ---
        ('P_2 (k=2)', 
         'M1^2 - (2*n + 2)*M1 + (n^2 + n + 1)'),
         
        ('P_3 (k=3)', 
         'M1^3 - 3*(n + 1)*M1^2 + 3*(n^2 + n + 1)*M1 - (n^3 + n^2 + n + 1)'),

        ('P_4 (k=4)', 
         'M1^4 - 4*(n + 1)*M1^3 + 6*(n^2 + n + 1)*M1^2 - 4*(n^3 + n^2 + n + 1)*M1 + (n^4 + n^3 + n^2 + n + 1)')
